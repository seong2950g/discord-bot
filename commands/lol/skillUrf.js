...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('우르프스킬')
		.setDescription('우르프 추천 스킬 정보를 제공합니다.')
    ...
    async execute(interaction) {
        ...
        const { championInfo, skillList } = await this.getSkillUrf(engChampionName);
        ...
        .setURL(`https://www.op.gg/modes/urf/${engChampionName}/skills?region=${info.region}&tier=${info.tier}&patch=${info.patch}`)
        .setColor("#00b0f4")
    ...
    async getSkillUrf(engChampionName) {
        const URL = `https://www.op.gg/modes/urf/${engChampionName}/skills?region=${info.region}&tier=${info.tier}&patch=${info.patch}`;
        ...
        try {
            ...
            const sortSkillList = skillContainer.sort((a, b) => {
                const valueA = parseFloat($(a).find('div > span.win-rate > em').text().trim());
                const valueB = parseFloat($(b).find('div > span.win-rate > em').text().trim());
