...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('칼바람룬')
		.setDescription('칼바람 추천 룬 정보를 제공합니다.')
    ...
    async execute(interaction) {
        ...
        const URL = `https://www.op.gg/modes/aram/${engChampionName}/runes?region=${info.region}&tier=${info.tier}&patch=${info.patch}`;
        interaction.channel.send("룬 정보를 가져오고 있습니다. 약 5초의 시간이 소요 됩니다.");

        const { championInfo, attachment, runeWinRate, runePickRate, runeGameCnt } = await this.getRuneAram(engChampionName);
    ...
    async getRuneAram(engChampionName) {
        console.time("getRuneAram");
        const URL = `https://www.op.gg/modes/aram/${engChampionName}/runes?region=${info.region}&tier=${info.tier}&patch=${info.patch}`;
        
        try {
            // 브라우저 열기 
            console.time("pageOpen");
            const browser = await puppeteer.launch({ headless: true });
            // 새로운 창 열기
            const page = await browser.newPage();
            console.timeEnd("pageOpen");
            ...
            const championTier = await page.$eval("#content-container > main > div.mode-header-container > div.aram.css-oadv09.e1qolk618 > div.inner > div.css-1rjiik1.ehsx5hv1 > div.img-box > div > img", img => img.getAttribute("alt"));
            const championWinRate = await page.$eval('#content-container > main > div.mode-header-container > div.aram.css-oadv09.e1qolk618 > div.inner > div.rate-info > div > div:nth-child(1) > div.css-oxevym.e1myk9su2', el => el.textContent.trim());
            const championPickRate = await page.$eval('#content-container > main > div.mode-header-container > div.aram.css-oadv09.e1qolk618 > div.inner > div.rate-info > div > div:nth-child(2) > div.css-oxevym.e1myk9su2', el => el.textContent.trim());
            const championInfo = {
