...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('칼바람아이템')
		.setDescription('칼바람 추천 아이템 정보를 제공합니다.')
    ...
    async execute(interaction) {
        ...
        const { championInfo, coreBuildList, shoeList, startItemList, itemList } = await this.getItemAram(engChampionName);
        ...
        .setURL(`https://www.op.gg/modes/aram/${engChampionName}/items?region=${info.region}&tier=${info.tier}&patch=${info.patch}`)
        .addFields({
            name: "아이템 통계",
            value: itemValue1,
            inline: true
    ...
    async getItemAram(engChampionName) {
        const URL = `https://www.op.gg/modes/aram/${engChampionName}/items?region=${info.region}&tier=${info.tier}&patch=${info.patch}`;
